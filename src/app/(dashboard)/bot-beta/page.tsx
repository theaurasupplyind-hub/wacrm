  ...
  if (result.pricing) {
    for (const item of result.pricing.items) {
      if (item.precio != null) {
        ...
        s += `❌ ${item.cantidad}x ${item.categoria} ${item.medida_solicitada}${item.categoria === 'BASTIDOR' && item.variante ? ` (${item.variante})` : ''} → SIN PRECIO\n`
      }
    }
                      ...
                      <div className="space-y-0.5">
                        {voiceResult.resolvedItems.map((item, i) => (
                          <p key={i} className="text-xs text-muted-foreground">
                            ...
                            {item.faltante
                              ? ' ❌ Sin referencia'
                              : ` → ${item.categoria} ${item.medida}${item.categoria === 'BASTIDOR' && item.variante ? ` (${item.variante})` : ''} $${item.precio_base?.toLocaleString('es-AR')}`
                            }
                        ...
                        <div key={i} className="space-y-1 border-b border-purple-500/10 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-foreground font-medium">
                              {item.cantidad}x {item.categoria} {item.medida_solicitada}{item.categoria === 'BASTIDOR' && item.variante ? ` (${item.variante})` : ''}
                            </span>
                            {item.precio != null ? (
